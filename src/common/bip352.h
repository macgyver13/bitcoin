...
struct secp256k1_silentpayments_recipient_public_data;
namespace bip352 {
...
{
private:
    std::unique_ptr<PublicDataImpl> m_impl;

public:
    PublicData();
    PublicData(PublicData&&) noexcept;
    PublicData& operator=(PublicData&&) noexcept;
    ~PublicData();

    // Delete copy constructors
    PublicData(const PublicData&) = delete;
    PublicData& operator=(const PublicData&) = delete;

    secp256k1_silentpayments_recipient_public_data* Get() const;
};
 ...
 * @return std::optional<PublicData> The silent payment tweak data, nullopt if not found.
 */
 ...
 */
std::optional<std::vector<SilentPaymentOutput>> ScanForSilentPaymentOutputs(const CKey& scan_key, const PublicData& public_data, const CPubKey& spend_pubkey, const std::vector<XOnlyPubKey>& output_pub_keys, const std::map<CPubKey, uint256>& labels);
}; // namespace bip352
